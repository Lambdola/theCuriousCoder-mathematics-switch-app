document.getElementById('calculate').addEventListener('click', function () {
    ...
    switch (operator) { 
        ...
        case 'mode':
            let mode = [], itemCount = [], len = 0;
            let inputSet = new Set(list);
            for (var items of inputSet) {
                for (var j = 0; j < list.length; j++) {
                    if ((items === list[j])) {
                        len++;
                    }
                }
                mode.push({"Number": items, "Count": len});
                itemCount.push(len);
                len = 0;
            }
            let countMax = Math.max(...itemCount);
            itemCount = mode.filter(items => items.Count === countMax);
            mode = [];
            for (items of itemCount) {
                mode.push(items.Number)
            }
